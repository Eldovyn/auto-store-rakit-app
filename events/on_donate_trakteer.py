...
class OnDonateTrakteer(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if not (ch := await ChannelDonateDatabase().get()):
            return
        if not message.channel.id == ch.trakteer:
            return
        ...
        amount_match = re.search(r"\b\d+\b(?=\s+dengan)", description)
        ...
        if not name or not rupiah:
            return
        try:
            rupiah = int(amount) if amount else None
        except:
            return
...
def setup(bot):
    bot.add_cog(OnDonateTrakteer(bot))
